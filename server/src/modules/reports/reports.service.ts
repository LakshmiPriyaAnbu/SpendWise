...
export async function getSummary(userId: string, range: ReportRange): Promise<ReportSummary> {
...
export async function exportReport(userId: string, input: ExportInput): Promise<ExportFile> {
  const { start, end, label } = rangeBounds(input.range);
  const tx = await prisma.transaction.findMany({
    where: { userId, date: { gte: start, lt: end } },
    include: { category: true },
    orderBy: { date: 'desc' },
  ...
  const name = `SpendWise-${label}.${input.format}`;
  let file: Buffer;
  ...
  await prisma.reportExport.create({
    data: { userId, name, format: input.format, size },
  });

  return { name, file, contentType };
}

export async function getHistory(userId: string): Promise<ReportHistoryItem[]> {
