...
import { monthBounds, toCategory, toTransaction } from '../../lib/serialize';
...
export async function getSummary(userId: string, month: string): Promise<AnalyticsSummary> {
  ...
  const [categories, monthTx, prevTx, budgets, allUpToEnd, trendTx] = await Promise.all([
    ...
    prisma.transaction.findMany({ where: { userId, date: { gte: start, lt: end } }, orderBy: [{ date: 'desc' }, { createdAt: 'desc' }] }),
    prisma.transaction.findMany({ where: { userId, date: { gte: prevStart, lt: start } } }),
    prisma.budget.findMany({ where: { userId, month } }),
    prisma.transaction.aggregate({ where: { userId, date: { lt: end } }, _sum: { amount: true } }),
    prisma.transaction.findMany({ where: { userId, date: { gte: trendStart, lt: end }, amount: { lt: 0 } }, select: { date: true, amount: true } }),
  ]);
